...
module StaticAnalysis.CFGTraverser where
...
data StackInfo = StackInfo {max_depth :: Int, calls :: [(ErrLoc, String)], true_while :: Bool, checking_while :: Bool}

initProg = initState (StackInfo search_depth [] False False)
...
pushPop new_env inserts stack_el f = do
  s <- get
  let StackInfo {max_depth = md, calls = st, true_while = tw, checking_while = cw} = dat s
  put $ SymTable (global_env s) (M.empty : new_env) (state s) (StackInfo {max_depth = md, calls = (stack_el : st), true_while=tw, checking_while = cw}) (loc s)
...
pushPop'' stack_el f = do
  s <- get
  let StackInfo {max_depth = md, calls = st, true_while = tw, checking_while = cw} = dat s
  put $ SymTable (global_env s) (M.empty : current_env s) (state s) (StackInfo {max_depth = md, calls = (stack_el : st), true_while = tw, checking_while = cw}) (loc s)
  ret <- f
  s2 <- get
...
pushPop''' :: MonadState (SymTable s StackInfo) m => m b -> m b
pushPop''' f = do
  s <- get
  let StackInfo {max_depth = md, calls = st, true_while = tw, checking_while = cw} = dat s
  put $ SymTable (global_env s) (M.empty : current_env s) (state s) (StackInfo {max_depth = md, calls = st, true_while = True, checking_while = cw}) (loc s)
  ret <- f
  s2 <- get
  put $ SymTable (global_env s2) (current_env s) (state s2) (dat s) (loc s)
  return ret
...
pushPop'''' f = do
  s <- get
  let StackInfo {max_depth = md, calls = st, true_while = tw, checking_while = cw} = dat s
  put $ SymTable (global_env s) (M.empty : current_env s) (state s) (StackInfo {max_depth = md, calls = st, true_while = tw, checking_while = True}) (loc s)
  ret <- f
  s2 <- get
  put $ SymTable (global_env s2) (current_env s) (state s2) (dat s) (loc s)
  return ret
...
---    state=state, dat=StackInfo{max_depth=max_depth, calls=calls, true_while=b, checking_while=checking_while}, loc=loc}
---    return ret
...
isCheckingWhile :: Traverser Bool
isCheckingWhile = do
  StackInfo{..} <- gets getData
  return checking_while